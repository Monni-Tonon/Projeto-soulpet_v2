        ...
        if (produto) {
            ...
            if ((categoria) && (desconto) && (dataDesconto)) {
              if (desconto < 0 || desconto > 100) {
                res
                  .status(400)
                  .json({ message: "Porcentagem de desconto inválida" });
              }

            // ---------------------- CORREÇÃO FUSO HORARIO ----------------------------------------
            // Verifica se a data de desconto é válida
            const currentDateUTC = new Date().getTime();
            const dataDescontoUTC = new Date(new Date(dataDesconto).toLocaleString("en-US", { timeZone: "America/Sao_Paulo" })).getTime();
                // comparando as variáveis tipo 'number' que representam a quantidade de milissegundos desde 01/01/1970 00:00:00 UTC
                if (dataDescontoUTC <= currentDateUTC) {
                    res.status(400).json({ message: "Data de desconto inválida" });
                return;
                }
              // -----------------------------------------------------------------------------------
